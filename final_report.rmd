...
In the chart treatment group, users answered correctly 85.7% of the time, while in the control they answered 83.3% of the time. **Table 1.**.
...
These were asked mainly as a distractor for subjects, prior to being presented with the questions of interest. The results are presented in **Table 2.**
...
The most basic model regresses the proportion of correct answers in both groups on :
...
We anticipated that the age of the participants might be a confounding factor and so regressed the test scores against the reported age from survey takers

\begin{table}[H] \centering 
  \label{Score regressed by Age} 
\begin{tabular}{@{\extracolsep{5pt}}lc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
 & \multicolumn{1}{c}{\textit{Dependent variable:}} \\ 
...
R$^{2}$ & 0.0002 \\ 
Adjusted R$^{2}$ & $-$0.004 \\ 
Residual Std. Error & 0.536 (df = 255) \\ 
F Statistic & 0.061 (df = 1; 255) \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{1}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
Another possible confounding factor we anticipated was whether participants had English as a first language.

\begin{table}[H] \centering 
  \label{Score regressed by ESL} 
\begin{tabular}{@{\extracolsep{5pt}}lc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
 & \multicolumn{1}{c}{\textit{Dependent variable:}} \\ 
...
R$^{2}$ & 0.019 \\ 
Adjusted R$^{2}$ & 0.008 \\ 
Residual Std. Error & 0.557 (df = 260) \\ 
F Statistic & 1.707 (df = 3; 260) \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{1}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
Finally, we built a fully saturated model including all of our confounding factors:
...
\hline \\[-1.8ex] 
 ...
 q2\_treat & 0.097 \\ 
  & (0.069) \\ 
  & \\ 
 as.factor(ESL == "Yes") & $-$0.012 \\ 
  & (0.075) \\ 
  & \\ 
 Birthyear & 0.00001 \\ 
  & (0.007) \\ 
  & \\ 
 as.factor(Edu\_Level)Bachelor's degree & $-$0.031 \\ 
  & (0.141) \\ 
  & \\ 
 as.factor(Edu\_Level)No college & $-$0.073 \\ 
  & (0.165) \\ 
  & \\ 
 as.factor(Edu\_Level)Some college & 0.063 \\ 
  & (0.137) \\ 
  & \\ 
 Constant & 1.600$^{***}$ \\ 
  & (0.245) \\ 
...
R$^{2}$ & 0.020 \\ 
Adjusted R$^{2}$ & $-$0.008 \\ 
Residual Std. Error & 0.537 (df = 249) \\ 
F Statistic & 0.718 (df = 7; 249) \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{1}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
